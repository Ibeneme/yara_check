...
const ShareholderROI = () => {
  ...
  const requestWithdrawal = async (distributionId: string, amount: number) => {
    try {
      const { error } = await supabase.from("roi_withdrawal_requests").insert({
  ...
  const getRequestStatus = (distributionId: string) => {
    return myRequests?.find((req) => req.distribution_id === distributionId)
      ?.status;
  };
  ...
  const totalReceived =
    myRequests
      ?.filter((req) => ["sent", "completed"].includes(req.status))
      .reduce((sum, req) => sum + parseFloat(req.amount.toString()), 0) || 0;

  // Available to withdraw
  const availableToWithdraw =
    distributions
      ?.filter((d) => d.withdrawal_enabled && !hasRequestForDistribution(d.id))
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-4">
                      ...
                      <div className="flex justify-between items-start">
                        <div>
                          <h4 className="font-medium">
                            ${distribution.amount} ({distribution.percentage}%
                            ROI)
                          </h4>
                          <p className="text-sm text-muted-foreground">
                            {distribution.period_type} -{" "}
                            {distribution.period_start} to{" "}
              ...
              {distributions.filter((d) => d.withdrawal_enabled).length ===
                0 && (
                ...
                <div key={distribution.id} className="border rounded-lg p-4">
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">
                        {distribution.period_type} - {distribution.period_start}{" "}
                        to {distribution.period_end}
                      </p>
                      <p className="text-xs text-muted-foreground">
                        Distributed:{" "}
                        {format(new Date(distribution.created_at), "PPP")}
                    ...
                    <Badge
                      variant={
                        distribution.withdrawal_enabled
                          ? "default"
                          : "secondary"
                      }
                    >
                ...
                <div key={request.id} className="border rounded-lg p-4">
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">
                        {request.distribution?.period_type} ROI (
                        {request.distribution?.percentage}%)
                      </p>
                      <p className="text-xs text-muted-foreground">
                        Requested:{" "}
