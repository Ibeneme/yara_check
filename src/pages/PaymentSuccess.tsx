...
const PaymentSuccess = () => {
  ...
  useEffect(() => {
    const processPaymentSuccess = async () => {
      ...
      console.log(
        "[STRIPE-SUCCESS] Full URL params:",
        Object.fromEntries(searchParams.entries())
      );
      console.log(
        "[STRIPE-SUCCESS] Checking localStorage for pending reports..."
      );
      ...
      try {
        console.log(
          "[STRIPE-SUCCESS] Starting payment processing for session:",
          sessionId
        );
        ...
        const pendingVehicleReport = localStorage.getItem(
        ...
        if (pendingPersonReport) {
          ...
          console.log("[STRIPE-SUCCESS] Person report data:", {
          ...
          if (reportData.image) {
            ...
            imageFile = new File([uint8Array], reportData.image.name, {
          ...
          if (savedPerson) {
            console.log(
              "[STRIPE-SUCCESS] Person saved successfully with tracking code:",
              code
            );
            ...
            console.log(
              "[STRIPE-SUCCESS] Redirecting to confirmation with code:",
              code
            );
          ...
          console.log("[STRIPE-SUCCESS] Device report data:", {
          ...
          const code = generateTrackingCode();
          const { saveDeviceToSupabase } = await import(
            "@/utils/supabaseStorage"
          );
          ...
          if (savedDevice) {
            console.log(
              "[STRIPE-SUCCESS] Device saved successfully with tracking code:",
              code
            );
          ...
          console.log("[STRIPE-SUCCESS] Vehicle report data:", {
            type: reportData.type,
          ...
          const code = generateTrackingCode();
          const { saveVehicleToSupabase } = await import(
            "@/utils/supabaseStorage"
          );
          ...
          if (savedVehicle) {
            console.log(
              "[STRIPE-SUCCESS] Vehicle saved successfully with tracking code:",
              code
            );
          ...
          if (storedTrackingCode) {
            console.log(
              "[STRIPE-SUCCESS] Using stored tracking code:",
              storedTrackingCode
            );
            ...
            console.error(
              "[STRIPE-SUCCESS] No pending report found after payment"
            );
            ...
            toast.success(
              "Payment successful! Your report has been submitted."
            );
            window.location.href = `/payment-confirmation?code=${code}`;
          }
        }
        ...
        toast.error(
          "Payment successful but report submission failed. Please contact support."
        );
        navigate("/support");
      } finally {
