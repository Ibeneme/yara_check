...
serve(async (req) => {
  ...
  try {
    ...
    console.log("Creating Flutterwave account payment:", { amount, trackingCode });
    ...
    const paymentReference = `FLW_ACCOUNT_${Date.now()}_${Math.random().toString(36).substring(7)}`;
    ...
    const flutterwavePayload = {
      ...
      customizations: {
        ...
        description: `Payment for hacked account report - ${reportData.account_type}`,
        logo: ""
      }
    };
    ...
    console.error("Flutterwave account payment error:", error);
    return new Response(
