...
serve(async (req) => {
  ...
  try {
    ...
    console.log("Creating Flutterwave device payment:", { amount, trackingCode });
    ...
    const paymentReference = `FLW_DEVICE_${Date.now()}_${Math.random().toString(36).substring(7)}`;
    ...
    const flutterwavePayload = {
      ...
      customizations: {
        title: "Stolen Device Report",
        description: `Payment for stolen device report - ${reportData.brand} ${reportData.model}`,
        logo: ""
      }
    };
    ...
    console.error("Flutterwave device payment error:", error);
    return new Response(
