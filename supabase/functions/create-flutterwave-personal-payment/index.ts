...
serve(async (req) => {
  ...
  try {
    ...
    console.log("Creating Flutterwave personal payment:", { amount, trackingCode });
    ...
    const paymentReference = `FLW_PERSONAL_${Date.now()}_${Math.random().toString(36).substring(7)}`;
    ...
    const flutterwavePayload = {
      ...
      customizations: {
        title: "Personal Belonging Report",
        description: `Payment for personal belonging report - ${reportData.brand} ${reportData.model}`,
        logo: ""
      }
    };
    ...
    console.error("Flutterwave personal payment error:", error);
    return new Response(
