...
serve(async (req) => {
  ...
  try {
    ...
    console.log("Creating Flutterwave reputation payment:", { amount, trackingCode });
    ...
    const paymentReference = `FLW_REPUTATION_${Date.now()}_${Math.random().toString(36).substring(7)}`;
    ...
    const flutterwavePayload = {
      ...
      customizations: {
        title: "Business Reputation Report",
        description: `Payment for business reputation report - ${reportData.reported_person_name}`,
        logo: ""
      }
    };
    ...
    console.error("Flutterwave reputation payment error:", error);
    return new Response(
