...
serve(async (req) => {
  ...
  try {
    ...
    console.log("Creating Paystack device payment:", { amount, trackingCode });
    ...
    const paymentReference = `PAYSTACK_DEVICE_${Date.now()}_${Math.random().toString(36).substring(7)}`;
    ...
    console.log("Paystack device payment initialized:", paystackData.data.reference);
    ...
    console.error("Paystack device payment error:", error);
    return new Response(
      JSON.stringify({ 
