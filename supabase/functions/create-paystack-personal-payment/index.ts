...
serve(async (req) => {
  ...
  try {
    ...
    console.log("Creating Paystack personal payment:", { amount, trackingCode });
    ...
    const paymentReference = `PAYSTACK_PERSONAL_${Date.now()}_${Math.random().toString(36).substring(7)}`;
    ...
    console.log("Paystack personal payment initialized:", paystackData.data.reference);
    ...
    console.error("Paystack personal payment error:", error);
    return new Response(
      JSON.stringify({ 
