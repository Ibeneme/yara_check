...
serve(async (req) => {
  ...
  try {
    ...
    console.log("Creating Paystack reputation payment:", { amount, trackingCode });
    ...
    const paymentReference = `PAYSTACK_REPUTATION_${Date.now()}_${Math.random().toString(36).substring(7)}`;
    ...
    console.log("Paystack reputation payment initialized:", paystackData.data.reference);
    ...
    console.error("Paystack reputation payment error:", error);
    return new Response(
      JSON.stringify({ 
