...
serve(async (req) => {
  ...
  try {
    ...
    console.log("Creating Paystack vehicle payment:", { amount, trackingCode });
    ...
    const paymentReference = `PAYSTACK_VEHICLE_${Date.now()}_${Math.random().toString(36).substring(7)}`;
    ...
    const paystackPayload = {
      ...
      metadata: {
        service: 'yaracheck_vehicle_report',
        report_type: 'vehicle',
        tracking_code: trackingCode
      }
    };
    ...
    console.log("Paystack vehicle payment initialized:", paystackData.data.reference);
    ...
    console.error("Paystack vehicle payment error:", error);
    return new Response(
      JSON.stringify({ 
