...
serve(async (req) => {
  ...
  try {
    ...
    // Get request body
    const { amount = 800 } = await req.json(); // Default to $8.00 in cents
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [
        {
          price_data: {
            currency: 'usd',
            product_data: {
              name: 'YaraCheck Vehicle Report Submission',
              description: 'Fee for submitting a vehicle report to YaraCheck verification system',
            },
            unit_amount: amount,
          },
          quantity: 1,
      ...
      cancel_url: `${req.headers.get("origin")}/submit-report/vehicle`,
      metadata: {
