...
serve(async (req) => {
  ...
  try {
    ...
    const verifyResponse = await fetch(`https://api.flutterwave.com/v3/transactions/verify_by_reference?tx_ref=${txRef}`, {
      method: "GET",
      headers: {
      ...
      switch (transaction.report_type) {
        case 'person':
          const personData = {
            ...
            physical_attributes: reportData.physical_attributes || '',
            description: reportData.description || '',
            location: reportData.location,
            date_missing: reportData.date_missing,
        ...
        case 'reputation':
          ...
          break;
        default:
          throw new Error(`Unknown report type: ${transaction.report_type}`);
      }
